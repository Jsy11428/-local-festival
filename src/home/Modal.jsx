...
const Modal = (props) =>{
    ...
    useEffect(() =>{        
        ...
        let themeDBObj = JSON.parse(localStorage.getItem("themeDB"));   //테마 DB의 더미 데이터 들을 파싱한다.
        let localDBObj = JSON.parse(localStorage.getItem("localDB"));   //지역 DB의 더미 데이터 들을 파싱한다.
        let infoArr = [];       //임시로 배열 정보를 받아올 배열형 변수
        console.log(localDBObj);
        if(themeDBObj[props.keyValue] !== undefined)        //테마db에 부모에게 받아온 키값이 있으면
        {
        ...
        else if(localDBObj[props.keyValue] !== undefined)       //로컬db에 부모에게 받온 키값이 있으면
        {
            console.log("localDBObj");
